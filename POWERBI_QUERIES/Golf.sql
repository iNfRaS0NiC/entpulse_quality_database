...
-- cross-sport reference that resolves rather than failing is the same shape the truncated Event
-- id list produces in a different layer, which `DATABASE.md` `DB-SEM-011` owns. Nothing reports
-- that one any more: `Golf-DQ-098` did until it was deprecated on 2026-08-20.
--
...
SELECT
    -- CheckID - Golf-DQ-126
    -- Name - COMP.RANK_RANKS_COMPETITORS_THE_TOURNAMENT_NEVER_ENTERED
    -- What it does: Finds Comp.Rank statistics ranking competitors who are entered in no event of their own tournament.
    'PARTICIPANT_NOT_IN_TOURNAMENT' AS check_type,
    x.statistic_id,
    x.statistic_name,
    x.template_name,
    ...
    x.stray_participants,
    x.stray_participant_rows,
    x.participant_ids,
    x.participant_names,
    NULL AS eligible_count,
    0 AS sort_order
-- What it does, stated in full: GLOBAL-DQ-030 rewritten for Golf because it cannot be run here.
-- It is the reverse of Golf-DQ-096, and the pair is the whole question asked both ways: -096
-- finds a competitor who played and was left out of the ranking, this finds one who is ranked
-- and never entered anything.
--
-- The template asks, for every ranked competitor, whether that person appears anywhere under
-- their tournament - as two correlated NOT EXISTS subqueries, evaluated once per ranked row.
-- Measured 2026-08-28 on Golf it did not return: 504 from the gateway. That is the same shape
-- and the same outcome as GLOBAL-DQ-042, which Golf-DQ-096 replaced for the same reason, and
-- the fix here is the fix there. Every (tournament, competitor) pair the sport entered is built
-- once as one set and joined; nothing is looked up per row.
--
-- **The lineup half of the template is dropped rather than translated, and that is a fact about
-- this sport rather than a shortcut.** GLOBAL-DQ-030 accepts a ranked athlete who appears only
-- as a lineup member, because a team sport enters the team and carries its people in lineups.
-- Golf writes no lineup row at all - SPORTS/Golf.md records that a foursome is stored as one
-- athlete participant holding both names, and measured the same day the sport holds 0 lineup
-- rows across every template. A join that can match nothing is a pass over the data for no
-- reading, so it is not made; if a lineup row ever appears here, this statement is where the
-- second set belongs.
--
-- The audited object is the statistic, as in the template and for its reason: whoever fixes
-- this works a ranking at a time, and one import that strayed is one thing wrong however many
-- people it listed. The two lists are a convenience and the counts beside them are the
-- assertion - the server truncates GROUP_CONCAT at 1024 characters without saying so, and the
-- ids are the ones to trust first because they are shorter.
-- ranked_participants is projected beside stray_participants so the proportion can be read on
-- the row: a ranking with two strays out of a hundred and one with two out of two are different
-- repairs and the template could not tell them apart.
FROM (
    SELECT
        s.id AS statistic_id,
        s.name AS statistic_name,
        tt.name AS template_name,
        t.name AS tournament_name,
        COUNT(DISTINCT sp.participantFK) AS ranked_participants,
        COUNT(DISTINCT CASE WHEN played.participant_id IS NULL
                            THEN sp.participantFK END) AS stray_participants,
        COUNT(DISTINCT CASE WHEN played.participant_id IS NULL
                            THEN sp.id END) AS stray_participant_rows,
        GROUP_CONCAT(DISTINCT CASE WHEN played.participant_id IS NULL THEN p.id END
                     ORDER BY p.id SEPARATOR ' | ') AS participant_ids,
        GROUP_CONCAT(DISTINCT CASE WHEN played.participant_id IS NULL THEN p.name END
                     ORDER BY p.name SEPARATOR ' | ') AS participant_names
    FROM statistic s
    JOIN tournament t ON t.id = s.objectFK AND t.del = 'no'
    JOIN tournament_template tt ON tt.id = t.tournament_templateFK AND tt.del = 'no'
         AND tt.sportFK = 3
    ...
    JOIN participant p ON p.id = sp.participantFK AND p.del = 'no'
    LEFT JOIN (
        -- Every (tournament, competitor) pair the sport entered, resolved once for the whole
        -- sport rather than asked per ranked row. This is the join that replaces the
        -- template's correlated NOT EXISTS, and it is the only difference that matters.
        SELECT DISTINCT
            ts2.tournamentFK AS tournament_id,
            ep2.participantFK AS participant_id
        FROM tournament_stage ts2
        JOIN tournament t2 ON t2.id = ts2.tournamentFK AND t2.del = 'no'
        JOIN tournament_template tt2 ON tt2.id = t2.tournament_templateFK AND tt2.del = 'no'
             AND tt2.sportFK = 3
      ...
      ON played.tournament_id = t.id
     AND played.participant_id = sp.participantFK
    WHERE s.del = 'no'
      AND s.statistic_typeFK = 11
      AND s.object_typeFK = 3
      AND (tt.name IS NULL OR tt.name NOT LIKE '%(IOC)%')
