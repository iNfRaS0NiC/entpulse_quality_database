...
-- ================================================================================
SELECT
    -- CheckID - Artistic-Gymnastics-DQ-126
...
--
-- Measured 2026-09-09: 146 of 7941 events. 85 put the gender before the discipline, 43 name
-- the apparatus Vault Apparatus where the discipline is Vault, and 18 carry no gender word at
-- all. The client scope removes none of them; all 7941 events are inside it.
FROM (
    SELECT
        x.*,
        TRIM(CONCAT_WS(' ', x.discipline, x.gender_label, x.round_word)) AS expected_base,
        TRIM(CONCAT_WS(' ', x.discipline, x.gender_label, x.round_word_alt)) AS expected_base_alt,
    ...
    FROM (
        SELECT
            ...
            END AS gender_label,
            -- The round words Artistic Gymnastics actually uses, keyed on the round type's
            -- own id. Six ids cover every active event in the sport.
            CASE rt.id
                WHEN 173 THEN 'Final'
            ...
            END AS round_word,
            -- Round type 173 carries two words in this sport rather than one. Measured
            -- 2026-09-09: of the 146 tournament / discipline / gender groups holding a
            -- Combined event, 145 also hold a separate Final one, so the two are different
            -- events told apart by name alone, and renaming Combined to Final would collide
            -- with an event already there. Accepting both is what keeps this check from
            -- asking for that.
            CASE rt.id WHEN 173 THEN 'Combined' END AS round_word_alt,
...
WHERE BINARY y.actual_base <> BINARY y.expected_base
           AND (y.round_word_alt IS NULL
                OR BINARY y.actual_base <> BINARY y.expected_base_alt)
...
-- ================================================================================
SELECT
    -- CheckID - Artistic-Gymnastics-DQ-127
...
-- made yet - the six ids in the list are the ones this sport uses today, and a new one
