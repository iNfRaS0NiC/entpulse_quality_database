...
-- reaches the place it belongs to.
    ...
    NULL AS eligible_count,
    0 AS sort_order
-- GLOBAL-DQ-026 narrowed twice, to the rankings this sport actually awards medals in.
-- The template audits every Comp.Rank a sport holds. Ice Hockey holds three kinds and only one
-- of them awards anything. Measured 2026-08-20 inside the client's 2004 boundary, the plain
-- instantiation returned 295 of 557 and 292 of those were two branches describing correct
-- data: 210 'No_Medals_At_All', almost all of them group-stage tables, and 82
-- 'Medal_Set_Unreadable_Without_Rank', almost all of them squad lists. Three rows said
-- something. Golf hit the same wall at 3286 of 3491 and answered it the same way, in its own
-- file rather than in the template, because six other sports run the template correctly.
--
-- The first narrowing is the template list, and it is this sport's MEDAL_TEMPLATE_ID_LIST
-- written out. It removes the Winter Olympics qualification tournaments and the Euro Hockey
-- Tour legs - Carlson and Beijer Hockey Games among them - which are contested and ranked and
-- award nobody a medal.
--
-- The second narrowing is by the ranking's own name, and that is not a shortcut taken for
-- convenience. Nothing in the structure separates a group table from the tournament's medal
-- table: measured that day, both carry the same three config fields - Start date, End date and
-- Gender - and both reach the same object_relation target types 33, 43 and 151. The name is
-- the only thing that differs, and the sport writes it consistently: 'Competition Stats
-- Group A' for a group, '(athletes)' for a squad list, and the tournament's own name for the
-- ranking that awards the medals. SPORTS/Swimming.md sets the precedent for reading a name
-- where the sport marks a distinction no other way.
--
-- What is excluded is not thereby declared correct. A group table holding a medal, or a squad
-- list holding one, is a real defect and this statement cannot see it. GLOBAL-DQ-125 asserts
-- the negative half - no medal outside the medal templates - and Ice-Hockey-DQ-083 held the
-- global template and is deprecated.
FROM (
    SELECT
        s.id AS statistic_id,
        s.name AS statistic_name,
